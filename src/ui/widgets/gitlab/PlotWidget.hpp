...
class PlotWidget : public Gtk::DrawingArea {
 private:
    ...
    size_t curIndex{0};
    size_t curSize{0};
    using pointArrType_t = std::array<double, MAX_POINT_COUNT>;
 ...
 private:
    void prep_widget();
    void draw_data(const Cairo::RefPtr<Cairo::Context>& ctx, int width, int height, const pointArrType_t& points, double maxVal, const Gdk::RGBA& color) const;
